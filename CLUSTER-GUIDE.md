...
**Data retention**: Prometheus stores metrics in `/var/lib/prometheus2/data/` on node1. Default retention is 15 days. This data does not require backups - metrics are ephemeral operational data that can be regenerated by the running cluster. If node1 fails, simply redeploy and metrics will start collecting again.
...
├── gitops.nix                # Auto-deploy service (15s polling)
...
| `gitops.nix` | Auto-deploy service that pulls from GitHub every 15 seconds |
...
K3s normally bundles several optional components. We disable them to save resources:

| Component | What it does | Why disabled |
|-----------|--------------|--------------|
| **Traefik** | Ingress controller - routes external HTTP traffic to pods based on hostnames/paths | Uses ~100MB RAM per node. Overkill for a home cluster. We use NodePort instead. |
| **ServiceLB** | Provides LoadBalancer service type using host ports | No external load balancer in home network. NodePort suffices. |
...
**Security**: The token file has restrictive permissions:
```
-rw------- 1 root root 65 /etc/k3s/token
```

Only root can read it. This is critical because anyone with the token can join the cluster as a control plane node. The token is a 64-character random string generated during initial cluster setup.

**If the token is compromised**:
1. Generate a new token on node1
2. Update `/etc/k3s/token` on all nodes
3. Restart K3s on all nodes: `sudo systemctl restart k3s`

**Initial token setup** (already done for this cluster):
```bash
# Generate token (done once during initial setup)
openssl rand -hex 32 > /tmp/k3s-token

# Distribute to all nodes
for i in 1 2 3 4 5 6 7; do
  scp /tmp/k3s-token admin@node$i.local:/tmp/
  ssh admin@node$i.local "sudo mv /tmp/k3s-token /etc/k3s/token && sudo chmod 600 /etc/k3s/token"
done
```
...
### Upgrading NixOS Packages

This updates all packages to their latest versions within the current NixOS release (25.05).

```bash
# On desktop (has Nix installed)
cd ~/sysadmin/odroid-c4
...
for i in 1 2 3 4 5 6 7; do
  echo -n "node$i: "
  ssh admin@node$i.local "cat /run/current-system/nixos-version"
done
```

### Upgrading to a New NixOS Release

When a new NixOS release comes out (e.g., 25.05 → 25.11):

1. **Update flake.nix** to reference the new release:
   ```nix
...
3. **Test on one node first**:
   ...
   git commit -m "Upgrade to NixOS 25.11"
   git push

   # Deploy to node7 first (least critical)
   ssh admin@node7.local "sudo nixos-rebuild switch --flake 'git+ssh://...#node7' --refresh"

   # Verify it works
   ssh admin@node7.local "uname -a && kubectl get nodes"
   ```

4. **Roll out to remaining nodes** once node7 is verified.

5. **Rollback if needed**:
   ```bash
   ssh admin@node7.local "sudo nixos-rebuild switch --rollback"
   ```

### Upgrading K3s

K3s is bundled with nixpkgs, so it upgrades automatically with `nix flake update`. To check versions:

```bash
# Current K3s version
ssh admin@node1.local "k3s --version"

# What version is in nixpkgs
nix eval nixpkgs#k3s.version
```
...
- [ ] Ensure no critical workloads are running
...
- [ ] Verify pods are healthy: `kubectl get pods -A`
- [ ] Check monitoring: `curl http://node1.local:9090/-/healthy`
